...
.step-nav-item.inactive { opacity: 0.35; }
.step-nav-item.active { opacity: 1; }
.step-nav-item.completed { opacity: 0.7; }
.step-dot {
...
min-width: 24px; margin-top: 1px;
transition: all 0.2s;
}
.step-nav-item.active .step-dot {
background: white; border-color: white; color: var(–maroon);
}
.step-nav-item.completed .step-dot {
...
.section-heading {
font-family: ‘Playfair Display’, serif;
font-size: 36px; font-weight: 900; color: var(–charcoal);
...
.option-card:hover { border-color: var(–maroon); background: var(–maroon-light); }
...
border: 1.5px solid rgba(123,26,56,0.3);
margin-top: 1px;
transition: all 0.15s;
display: flex; align-items: center; justify-content: center;
}
...
.toggle-sub {
font-size: 13px; font-weight: 300; color: var(–mid); line-height: 1.5;
}
.toggle-switch {
width: 44px; height: 24px; background: rgba(123,26,56,0.15);
border-radius: 12px; cursor: pointer; position: relative;
transition: background 0.2s; min-width: 44px; margin-left: 16px;
border: none;
}
...
.btn-back:hover { color: var(–maroon); border-color: var(–maroon); }
.btn-next {
background: var(–maroon); border: none; cursor: pointer;
font-family: ‘Barlow Condensed’, sans-serif;
font-size: 15px; font-weight: 700; letter-spacing: 0.1em;
text-transform: uppercase; color: white;
padding: 16px 48px; transition: background 0.2s, transform 0.15s;
}
.btn-next:hover { background: var(–maroon-dark); transform: translateY(-1px); }
...
.success-icon {
width: 72px; height: 72px; background: var(–maroon);
...
.success-title {
font-family: ‘Playfair Display’, serif;
font-size: 42px; font-weight: 900; color: var(–charcoal);
margin-bottom: 16px;
}
...
const DIETARY = [
“None”, “No red meat”, “No pork”, “Pescatarian”,
...
function OptionCard({ selected, onClick, label, desc, multi = false }) {
return (
<div className={`option-card ${selected ? "selected" : ""}`} onClick={onClick}>
<div className={`option-check ${multi ? "" : "round"}`}>
<CheckIcon />
</div>
<div>
...
// Section 4
weight_management_goal: “”, dietary_restrictions: [],
foods_to_avoid: “”, food_preferences: “”, travel_frequency: “”, nutrition_approach: “”,
// Section 5
avg_sleep_hours: “”, sleep_issue: “”,
...
user_id: “00000000-0000-0000-0000-000000000001”, // placeholder until auth
})
});
...
<div className="sidebar-sub">We’ll notify you as soon as it’s ready.</div>
</div>
<div className="form-main">
<div className="success-screen">
<div className="success-icon">✓</div>
<div className="success-title">You’re all set, <em>{form.first_name || “athlete”}.</em></div>
<p className="success-sub">
Your personalized Rallis Regimen program is being generated. You’ll receive your training program, nutrition plan, sleep protocol, and environment guide shortly.
</p>
</div>
</div>
</div>
    ...
    <div className="form-main">
      ...
      {step === 0 && (
        <>
          ...
          <p className="section-desc">Basic information we use to calculate your calorie targets, macro needs, and program structure.</p>

          <div className="field-group">
            <label className="field-label">First Name <span className="field-required">*</span></label>
            <input type="text" value={form.first_name} onChange={e => set("first_name", e.target.value)} placeholder="Your first name" />
          </div>

          <div className="field-row">
            <div className="field-group">
              <label className="field-label">Age <span className="field-required">*</span></label>
              <input type="number" value={form.age} onChange={e => set("age", e.target.value)} placeholder="e.g. 34" min="16" max="90" />
            </div>
            <div className="field-group">
          ...
          <div className="field-group">
            ...
            <div className="field-row">
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input type="number" value={form.height_ft} onChange={e => set("height_ft", e.target.value)} placeholder="ft" min="4" max="7" style={{ width: "100%" }} />
                <span style={{ color: "var(--mid)", whiteSpace: "nowrap", fontFamily: "Barlow Condensed", fontSize: 13 }}>ft</span>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input type="number" value={form.height_in} onChange={e => set("height_in", e.target.value)} placeholder="in" min="0" max="11" style={{ width: "100%" }} />
                <span style={{ color: "var(--mid)", whiteSpace: "nowrap", fontFamily: "Barlow Condensed", fontSize: 13 }}>in</span>
              </div>
            </div>
          </div>
          ...
          <div className="field-row">
            <div className="field-group">
              ...
              <input type="number" value={form.current_weight_lbs} onChange={e => set("current_weight_lbs", e.target.value)} placeholder="e.g. 185" />
            </div>
            <div className="field-group">
              ...
              <input type="number" value={form.ideal_weight_lbs} onChange={e => set("ideal_weight_lbs", e.target.value)} placeholder="e.g. 175" />
            </div>
          </div>
        </>
      )}
      ...
      {step === 1 && (
        <>
          ...
          <p className="section-desc">Pick one primary goal and one secondary. Your program is built around your #1 priority — the secondary goal influences exercise selection and conditioning approach.</p>

          <div className="priority-block">
            <span className="priority-label">
              Primary Goal <span className="priority-badge">#1 Priority</span>
            </span>
            <div className="option-grid">
              {GOALS.map(g => (
                <OptionCard
                  key={g.id}
                  selected={form.goal_primary === g.id}
                  onClick={() => {
                    set("goal_primary", g.id);
                    if (form.goal_secondary === g.id) set("goal_secondary", "");
                  }}
          ...
          <div className="priority-block">
            <span className="priority-label">
              Secondary Goal <span className="priority-badge secondary">#2 Priority</span>
            </span>
            <span className="field-hint">Must be different from your primary goal.</span>
            <div className="option-grid">
              {GOALS.filter(g => g.id !== form.goal_primary).map(g => (
                <OptionCard
                  key={g.id}
                  selected={form.goal_secondary === g.id}
                  onClick={() => set("goal_secondary", g.id)}
                  label={g.label} desc={g.desc}
                />
              ))}
            </div>
      ...
      {step === 2 && (
        <>
          ...
          <p className="section-desc">This determines your program structure, split, and exercise selection. Be honest about your schedule — a realistic program you follow beats an ideal one you don't.</p>
          ...
          <div className="field-group">
            ...
            <div className="option-grid-3">
              {[
                ...
                <OptionCard key={o.id} selected={form.experience_level === o.id}
                  onClick={() => set("experience_level", o.id)} label={o.label} desc={o.desc} />
              ))}
            </div>
          </div>
          ...
          <div className="field-row">
            <div className="field-group">
              ...
              <span className="field-hint">How many days can you realistically commit to resistance training?</span>
              <select value={form.training_days_per_week} onChange={e => set("training_days_per_week", e.target.value)}>
                <option value="">Select...</option>
            ...
            <div className="field-group">
              ...
              <span className="field-hint">How long do you have per session?</span>
              <select value={form.session_length_mins} onChange={e => set("session_length_mins", e.target.value)}>
                <option value="">Select...</option>
          ...
          <div className="field-group">
            <label className="field-label">Equipment Access <span className="field-required">*</span></label>
            <div className="option-grid">
              {EQUIPMENT.map(o => (
                <OptionCard key={o.id} selected={form.equipment === o.id}
                  onClick={() => set("equipment", o.id)} label={o.label} desc={o.desc} />
              ))}
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <div className="option-grid">
              {["Barbell","Trap Bar","Safety Squat Bar","Dumbbells","Kettlebells","Cable Machines","Weight Machines","Resistance Bands","Pull-Up Bar"].map(e => (
                <OptionCard key={e} multi selected={(form.equipment_detail||[]).includes(e)}
                  onClick={() => toggleArr("equipment_detail", e)} label={e} />
              ))}
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <span className="field-hint">E.g. too much volume, overtraining, or following a program that wasn't sustainable.</span>
            <div className="option-grid-3">
              {[
                { id: "no", label: "No", desc: "Never had burnout issues" },
                { id: "yes_mild", label: "Somewhat", desc: "Felt overtrained a few times" },
                { id: "yes_severe", label: "Yes", desc: "Burned out and had to take significant time off" },
              ].map(o => (
                <OptionCard key={o.id} selected={form.burnout_history === o.id}
                  onClick={() => set("burnout_history", o.id)} label={o.label} desc={o.desc} />
              ))}
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <textarea value={form.injuries_limitations} onChange={e => set("injuries_limitations", e.target.value)}
              placeholder="e.g. Lower back issues, avoid heavy overhead pressing. Bad left knee." />
          </div>
        </>
      )}
      ...
      {step === 3 && (
        <>
          ...
          <p className="section-desc">Your nutrition plan is built around your training schedule, body composition goal, and food preferences — using the Rallis Regimen whole-food framework.</p>

          <div className="field-group">
            <label className="field-label">Weight Management Goal <span className="field-required">*</span></label>
            <div className="option-grid-3">
              {[
                { id: "bulk", label: "Gain Weight", desc: "Calorie surplus — maximize muscle and strength gains" },
                { id: "maintain", label: "Maintain", desc: "Calorie maintenance — recomp and performance" },
                { id: "cut", label: "Lose Weight", desc: "Calorie deficit — reduce body fat" },
              ].map(o => (
                <OptionCard key={o.id} selected={form.weight_management_goal === o.id}
                  onClick={() => set("weight_management_goal", o.id)} label={o.label} desc={o.desc} />
              ))}
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <div className="option-grid">
              {[
                ...
                <OptionCard key={o.id} selected={form.nutrition_approach === o.id}
                  onClick={() => set("nutrition_approach", o.id)} label={o.label} desc={o.desc} />
              ))}
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <span className="field-hint">This affects how your meal plan is structured.</span>
            <div className="option-grid">
              {[
                ...
                { id: "mostly", label: "Most of the time", desc: "Rarely cooking at home, mostly eating out or premade" },
              ].map(o => (
                <OptionCard key={o.id} selected={form.travel_frequency === o.id}
                  onClick={() => set("travel_frequency", o.id)} label={o.label} desc={o.desc} />
              ))}
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <span className="field-hint">Select all that apply. Your meal plan will work around these.</span>
            <div className="option-grid">
              {DIETARY.map(d => (
                <OptionCard key={d} multi selected={form.dietary_restrictions.includes(d)}
                  onClick={() => toggleArr("dietary_restrictions", d)} label={d} />
              ))}
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <span className="field-hint">Any preferences you want your meal plan to reflect? E.g. "Rotate protein sources," "No chicken every day," "Keep meals simple."</span>
            <textarea value={form.food_preferences} onChange={e => set("food_preferences", e.target.value)}
              placeholder="e.g. I like to rotate my protein sources. Keep meals simple when traveling." />
          </div>

          <div className="field-group">
            <label className="field-label">Foods to Avoid</label>
            <span className="field-hint">Anything you strongly dislike or want kept out of your meal plan?</span>
            <textarea value={form.foods_to_avoid} onChange={e => set("foods_to_avoid", e.target.value)}
              placeholder="e.g. Sardines, organ meats, kefir" />
          </div>
        </>
      )}
      ...
      {step === 4 && (
        <>
          ...
          <p className="section-desc">Sleep is the primary driver of recovery and adaptation. Your sleep protocol is built around your actual habits — not a generic recommendation.</p>

          <div className="field-row">
            <div className="field-group">
              <label className="field-label">Average Sleep Hours <span className="field-required">*</span></label>
              <input type="number" value={form.avg_sleep_hours} onChange={e => set("avg_sleep_hours", e.target.value)}
                placeholder="e.g. 6.5" min="3" max="12" step="0.5" />
            </div>
            <div className="field-group">
              <label className="field-label">Sleep Issues</label>
              <select value={form.sleep_issue} onChange={e => set("sleep_issue", e.target.value)}>
                <option value="">Select...</option>
          ...
          <div className="field-row">
            <div className="field-group">
              ...
              <input type="text" value={form.typical_bedtime} onChange={e => set("typical_bedtime", e.target.value)}
                placeholder="e.g. 10:30 PM" />
            </div>
            <div className="field-group">
              <label className="field-label">Typical Wake Time</label>
              <input type="text" value={form.typical_wake_time} onChange={e => set("typical_wake_time", e.target.value)}
                placeholder="e.g. 6:30 AM" />
            </div>
          </div>

          <div className="field-group" style={{ marginTop: 8 }}>
            <Toggle label="Caffeine after noon"
              sub="Coffee, pre-workout, tea, or energy drinks after 12pm"
              value={form.caffeine_after_noon}
              onChange={v => set("caffeine_after_noon", v)} />
            <Toggle label="Phone in bedroom"
              sub="Do you keep your phone in your bedroom while sleeping?"
              value={form.phone_in_bedroom}
              onChange={v => set("phone_in_bedroom", v)} />
          </div>
        </>
      )}
      ...
      {step === 5 && (
        <>
          ...
          <p className="section-desc">A quick audit of your current habits. This tells us where your highest-impact environment changes are — we'll prioritize the easy wins first.</p>

          <Toggle label="I filter my drinking water"
            sub="Reverse osmosis, pitcher filter, or other filtration system"
            value={form.filters_water}
            onChange={v => set("filters_water", v)} />
          <Toggle label="I get morning sunlight within an hour of waking"
            sub="Natural outdoor light, not through a window"
            value={form.morning_sunlight}
            onChange={v => set("morning_sunlight", v)} />
          <Toggle label="I cook with non-stick cookware"
            sub="Teflon or PFAS-coated pans"
            value={form.nonstick_cookware}
            onChange={v => set("nonstick_cookware", v)} />
          <Toggle label="I use conventional cleaning and laundry products"
            sub="Standard supermarket cleaners, detergents, and fabric softeners"
            value={form.conventional_cleaning}
            onChange={v => set("conventional_cleaning", v)} />
          <Toggle label="My phone charges in my bedroom overnight"
            sub="Phone within arm's reach while sleeping"
            value={form.phone_in_bedroom_sleeping}
            onChange={v => set("phone_in_bedroom_sleeping", v)} />
          ...
          <div className="field-group" style={{ marginTop: 32 }}>
            ...
            <textarea value={form.success_vision} onChange={e => set("success_vision", e.target.value)}
              placeholder='e.g. "I want to actually look as strong as I am." "I want to feel athletic again." "I want to lose 20 lbs and keep it off."'
              style={{ minHeight: 100 }} />
          </div>
      ...
      <div className="form-nav">
        ...
        {step < totalSteps - 1 ? (
          ...
          <button className="btn-next" onClick={handleSubmit} disabled={submitting}>
            {submitting ? "Submitting..." : "Build My Program →"}
          </button>
        )}
      </div>
