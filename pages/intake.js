...
const styles = `
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .option-card.selected { border-color:var(--maroon); background:var(--maroon-light); }
  ...
  .btn-back { background:transparent; border:none; cursor:pointer; font-family:'Barlow Condensed',sans-serif; font-size:14px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; color:var(--mid); padding:14px 0; border-bottom:1px solid var(--mid); transition:color 0.2s,border-color 0.2s; }
  .btn-back:hover { color:var(--maroon); border-color:var(--maroon); }
  .btn-next { background:var(--maroon); border:none; cursor:pointer; font-family:'Barlow Condensed',sans-serif; font-size:15px; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; color:white; padding:16px 48px; transition:background 0.2s,transform 0.15s; }
  ...
  .error-msg { color:var(--error); font-size:13px; margin-top:6px; font-family:'Barlow Condensed',sans-serif; font-weight:500; letter-spacing:0.04em; }
...
function OptionCard({ selected, onClick, label, desc, multi }) {
  return (
...
export default function IntakeForm() {
  ...
  const set = (key, val) => setForm(function(f) { return Object.assign({}, f, { [key]: val }); });

  const toggleArr = (key, val) => setForm(function(f) {
    var arr = f[key] || [];
    var next = arr.includes(val) ? arr.filter(function(v) { return v !== val; }) : arr.concat([val]);
        ...
        <div className="sidebar">
          ...
          <div className="steps-nav">
            {STEPS.map(function(s, i) {
              return (
                <div key={s.num} className={"step-nav-item " + (i === step ? "active" : i < step ? "completed" : "inactive")} onClick={function() { if (i < step) setStep(i); }}>
                  <div className="step-dot">{i < step ? "V" : s.num}</div>
        ...
        <div className="form-main">
          ...
          {step === 2 && (
            <>
              ...
              <div className="field-row">
                <div className="field-group">
                  ...
                  <select value={form.training_days_per_week} onChange={function(e) { set("training_days_per_week", e.target.value); }}>
                    <option value="">Select...</option>
                    {[2,3,4,5,6].map(function(n) { return <option key={n} value={n}>{n} days</option>; })}
                  </select>
                </div>
                <div className="field-group">
              ...
              <div className="field-group">
                ...
                <div className="option-grid">
                  {SPECIFIC_EQUIPMENT.map(function(item) {
                    return (
                      <OptionCard key={item} multi selected={(form.equipment_detail || []).includes(item)}
                        onClick={function() { toggleArr("equipment_detail", item); }} label={item} />
                    );
                  })}
                </div>
              </div>
              <div className="field-group">
                ...
                <span className="field-hint">Too much volume, overtraining, or following an unsustainable program.</span>
                <div className="option-grid-3">
                  {[
              ...
              <div className="field-group">
                ...
                <textarea value={form.injuries_limitations} onChange={function(e) { set("injuries_limitations", e.target.value); }} placeholder="e.g. Lower back issues, avoid heavy overhead pressing. Bad left knee." />
              </div>
            </>
          )}
              ...
              <div className="field-group">
                ...
                <div className="option-grid">
                  {DIETARY.map(function(d) {
                    return <OptionCard key={d} multi selected={form.dietary_restrictions.includes(d)} onClick={function() { toggleArr("dietary_restrictions", d); }} label={d} />;
                  })}
                </div>
              </div>
              <div className="field-group">
          ...
          {step === 5 && (
            <>
              ...
              <Toggle label="My phone charges in my bedroom overnight" sub="Phone within arm's reach while sleeping" value={form.phone_in_bedroom_sleeping} onChange={function(v) { set("phone_in_bedroom_sleeping", v); }} />
          ...
          <div className="form-nav">
            {step > 0 ? <button className="btn-back" onClick={function() { setStep(function(s) { return s - 1; }); }}>Back</button> : <span />}
            {step < totalSteps - 1 ? (
