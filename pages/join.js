...
var WAIVER = 'ASSUMPTION OF RISK AND LIABILITY WAIVER. By enrolling in the Rallis Regimen, I acknowledge that physical exercise involves inherent risks. I represent that I am in good physical health. I agree to consult a physician before beginning any exercise program. I voluntarily assume all risks and release Rallis Regimen from any liability for injury or damage arising from my participation.';
...
export default function JoinPage() {
  ...
  var s1 = useState('monthly'); var plan = s1[0]; var setPlan = s1[1];
  var s2 = useState(''); var name = s2[0]; var setName = s2[1];
  var s3 = useState(''); var email = s3[0]; var setEmail = s3[1];
  var s4 = useState(''); var password = s4[0]; var setPassword = s4[1];
  var s5 = useState(false); var waiver = s5[0]; var setWaiver = s5[1];
  var s6 = useState(''); var promo = s6[0]; var setPromo = s6[1];
  var s7 = useState(false); var promoOk = s7[0]; var setPromoOk = s7[1];
  ...
  async function handleJoin() {
    ...
    if (promoOk) {
      try {
        var signUpResult = await supabase.auth.signUp({ email: email, password: password, options: { data: { full_name: name } } });
    ...
    try {
      var res = await fetch('/api/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email, name: name, password: password, plan: plan }) });
      ...
      <div className='wrap'>
        <div className='left'>
          ...
          <div className='includes'>
            {INCLUDES.map(function(item, i) { return (<div className='item' key={i}><div className='chk'>V</div>{item}</div>); })}
          </div>
        </div>
        <div className='right'>
          ...
          <div className='fg'><label className='fl'>First Name</label><input type='text' value={name} onChange={function(e) { setName(e.target.value); }} placeholder='Your first name' /></div>
          <div className='fg'><label className='fl'>Email Address</label><input type='email' value={email} onChange={function(e) { setEmail(e.target.value); }} placeholder='you@example.com' /></div>
          <div className='fg'><label className='fl'>Create Password</label><input type='password' value={password} onChange={function(e) { setPassword(e.target.value); }} placeholder='At least 8 characters' /></div>
          <div className='fg'>
          ...
          <div className='waiver-box'><div className='waiver-text'><strong>Assumption of Risk and Liability Waiver</strong><br /><br />{WAIVER}</div></div>
          <div className='cb-row' onClick={function() { setWaiver(!waiver); }}>
            <div className={waiver ? 'cb-box on' : 'cb-box'}>
              {waiver && (<svg width='10' height='8' viewBox='0 0 10 8' fill='none'><path d='M1 4L3.5 6.5L9 1' stroke='white' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round'/></svg>)}
            </div>
            <div className='cb-label'>I have read and agree to the Liability Waiver and confirm I am in good health.</div>
          </div>
          ...
          <p className='note'>{promoOk ? 'Promo code applied. No payment required.' : 'Secure checkout via Stripe. Not charged for 30 days.'}</p>
